...
function configure(parser)
  ...
  --parser:argument("txDev", "Device to transmit from."):convert(tonumber)
	...
	parser:option("-t --tx-dev", "Device to transmit from."):default(0):convert(tonumber):target("txDev")
	parser:option("-r --rx-dev", "Device to receive from."):default(0):convert(tonumber):target("rxDev")
	...
	parser:option("-e --eth-dst", "Ethernet destination."):default("00:00:00:00:00:00"):convert(tostring):target("ethDst")
	parser:option("-n --num-flows", "Number of flows to generate."):default(1):convert(tonumber):target("numFlows")
	parser:option("-p --pkt-size", "Packet size in bytes."):default(1400):convert(tonumber):target("pktSize")
end
...
function master(args)
	...
	if args.bgRate > 0 then
		mg.startTask("loadSlave", txDev:getTxQueue(0), PORT_BG, args)
	end
	-- high priority traffic (different UDP port)
	if args.fgRate > 0 then
		mg.startTask("loadSlave", txDev:getTxQueue(1), PORT_FG, args)
	end
	-- count the incoming packets
	mg.startTask("counterSlave", rxDev:getRxQueue(0))
	-- measure latency from a second queue
	mg.startSharedTask("timerSlave", txDev:getTxQueue(2), rxDev:getRxQueue(1), PORT_BG, PORT_FG, args.fgRate / (args.fgRate + args.bgRate), args)
	-- wait until all tasks are finished
	mg.waitForTasks()
end

function loadSlave(queue, port, args)
	mg.sleepMillis(100) -- wait a few milliseconds to ensure that the rx thread is running
	-- TODO: implement barriers
	local mem = memory.createMemPool(function(buf)
		buf:getUdpPacket():fill{
			pktLength = args.pktSize, -- this sets all length headers fields in all used protocols
	...
	while mg.running() do
		...
		for _, buf in ipairs(bufs) do
			...
			pkt.ip4.src:set(baseIP + math.random(args.numFlows) - 1)
			-- you can modify other fields here (e.g. different source ports or destination addresses)
		end
		-- send packets
		bufs:offloadUdpChecksums()
		txCtr:updateWithSize(queue:send(bufs), args.pktSize)
	end
	txCtr:finalize()
end
...
function timerSlave(txQueue, rxQueue, bgPort, port, ratio, args)
	...
	while mg.running() do
		...
		local lat = timestamper:measureLatency(args.pktSize, function(buf)
			local pkt = buf:getUdpPacket()
			pkt:fill{
				pktLength = args.pktSize, -- this sets all length headers fields in all used protocols
